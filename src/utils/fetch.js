...
const fetchJson = async url => {
  try {
    return (await global.fetch(`${url}.json`, {
...
export const fetchStats = async controllaUrl => {
  try {
    ...
    report(e);
    report(
      `Could not load the stats for ${controllaSlugFromUrl(controllaUrl)}`
    );
  }
};
...
export const fetchLogo = async logoUrl => {
  ...
  try {
    ...
    report(
      `Error while fetching logo from ${logoUrl}. The response wasn't well-formatted`
    );
  } catch (e) {
...
const isLogoResponseWellFormatted = res =>
  res.status === 200 && res.headers.get("content-type").match(/^text\/plain/);
